...
<script>
	...
	import {authorizeJoin} from '@/api/serviceNumber.js'
	import {authorize} from '@/api/serviceNumber.js'
	...
	export default {
		...
		methods: {
			isLogin(){
				let param = _this.code;
				authorize(param).then(response => {
					if (response.status === 200) {
						_this.info =  response.data;
						let errorCode  = response.data.errorCode;
						switch (errorCode) {
						  case '100':
								_this.openId = response.data.body.userInfo.openid;
						  ...
						  case '200':
						    ...
						    break
						  case '300':
								this.enterpriseName = response.data.body.userInfo.client.name;
								this.phoneNumber  = response.data.body.userInfo.telephone;
								this.openId = response.data.body.userInfo.openId;
								this.isRotate  = true;
								uni.showToast({
								    icon: 'none',
									position: 'bottom',
		    ...
		    startLogin(){
		        ...
		        if (this.phoneNumber.length == "") {
		            ...
		            return;
		        }else if (!(/^(((13[0-9]{1})|(15[0-9]{1})|(18[0-9]{1}))+\d{8})$/.test(this.phoneNumber))) {
					uni.showToast({
					    icon: 'none',
						position: 'bottom',
					    title: '手机格式不正确'
				...
				userdata.append("clientName",_this.enterpriseName); // 公司名称（全称）
				userdata.append("telephone",_this.phoneNumber);   // 联系方式
				userdata.append("userName",_this.nickName);    // 微信名称
				userdata.append("openId",_this.openId);         // 微信公众号的OpenID
					...
					if (response.status === 200) {
						...
						switch (errorCode) {
						  case '1':
						    ...
						    break
						  case '2':
								this.labelText = response.data.msg;
							break
						  case '200':
								uni.showToast({
									icon: 'success',
